...
kbl(
    models_joined,
    booktabs = TRUE,
    caption = "Mixed effects model output",
...
kbl(
    emmeans_joined,
    booktabs = TRUE,
    caption = "Mixed effects model: estimated marginal means (female - male)",
    label = "tbl-emmeans"
) |>
...
We generated bootstrapped datasets by creating 1,000 new datasets of the size of the original data. We did this through non-parametric sampling of the original with replacement, taking samples at Document ID level to preserve the correlation of sentiment within documents [@carpenter2000bootstrap]. We then ran our original linear mixed model for each bootstrapped dataset. The bootstrapped estimates are the mean of all 1,000 estimates. The results for the SiEBERT model are in Table \ref{tab:tbl-final-sentiment-models-siebert-bootstrap} and the Regard results are in Table \ref{tab:tbl-final-sentiment-models-regard-bootstrap}.  We calculated the additional columns in Table {tab:tbl-final-sentiment-models-siebert-bootstrap} as follows:
...
kbl(
    out,
    booktabs = TRUE,
    caption = "Bootstrapped model output (SiEBERT)",
...
kbl(
    out,
    booktabs = TRUE,
    caption = "Bootstrapped model output (Regard)",
...
kbl(
    models_joined,
    booktabs = TRUE,
    caption = "Robust mixed effects model output",
...
kbl(
    emmeans_joined,
    booktabs = TRUE,
    caption = "Robust mixed effects model: estimated marginal means (female - male)",
...
where $\beta_0$ is the intercept, $\beta_1$, $\beta_2$, and $\beta_3$ are the coefficients for model, gender, and their interaction, $\beta_4$ is the coefficient for maximum tokens, $u_{0j}$ is the random intercept for document $j$ and $\epsilon_{ij}$ is the residual error term. We set out the results in Table \ref{tab:tbl-final-sentiment-models-nlme} and the estimated marginal means in Table \ref{tab:tbl-emmeansnlme}. The estimates and are very close to the output from the linear mixed model, though the $p$-values are slightly larger. The BART and T5 models are on the boundary of significance, but now the $p$-values are slightly larger than 0.05. Llama 3 has no significant differences in sentiment between men and women, and Gemma has the largest standardised estimates and smallest $p$-values. This model reduces the risk of Type 1 error, which we see in the larger $p$-values, so it is reassuring that our main findings about Llama 3 and Gemma remain consistent.  
...
kbl(
    models_joined,
    booktabs = TRUE,
    caption = "Variance-structured mixed effects model output",
...
kbl(
    emmeans_joined,
    booktabs = TRUE,
    caption = "Variance-structured mixed effects: estimated marginal means (female - male)",
...
We additionally used a Generalised Estimating Equations (GEE) model, which estimates population-averaged effects and adjusts for within-group correlation using robust sandwich estimators, using the `geepack` R package [@geepack]. This model estimates population-averaged effects and can be more robust to misspecified  correlation structures. The GEE model is specified as:
...
where the correlation structure of the residuals $\epsilon_{i}$ is modelled as exchangeable within groups defined by Document ID.  We do not apply corrections to the standard errors to reduce the risk of Type 1 error, as we have 617 document-level clusters, and generally with 100 or more clusters such corrections are not necessary [@kahan2016increased]. We set out the results of the GEE model in Table \ref{tab:tbl-final-sentiment-models-gee}. We set out the estimated marginal means for the GEE model in Table \ref{tab:tbl-emmeansgee}. 
...
kableExtra::kbl(models_joined, booktabs = TRUE, label = "tbl-final-sentiment-models-gee", caption = "GEE model output") |>
    kable_styling(latex_options = c("striped", "scale_down", "HOLD_position")) |>
    kableExtra::add_header_above(c(" " = 1, "Regard" = 5, "SiEBERT" = 5))
```
...
kbl(
    emmeans_joined,
    booktabs = TRUE,
    caption = "GEE model: estimated marginal means (female - male)",
...
kbl(
    emmeans_joined,
    booktabs = TRUE,
    caption = "Linear models: estimated marginal means (female - male)",
    label = "tbl-emmeanslm"
) |>
    kable_styling(latex_options = c("striped", "HOLD_position")) |>
    kableExtra::add_header_above(c(" " = 1, "Regard" = 4, "SiEBERT" = 4))
