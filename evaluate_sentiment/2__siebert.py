...
def load_siebert_model(model_name: str = "siebert/sentiment-roberta-large-english"):
    """
...
class SimpleDataset:
    ...
    See e.g. https://colab.research.google.com/github/chrsiebert/sentiment-roberta-large-english/blob/main/sentiment_roberta_prediction_example.ipynb
    """
...
def write_siebert_df(
    ...
    out_file_original, out_file_result = create_outfile_path(
        in_file, "siebert", out_dir
    )

    if out_file_original.is_file() and out_file_result.is_file():
        print(f"Siebert sentiment already calculated for {in_file.name}. Skipping.")
        return
    ...
    out_df_list_originals = [
        get_siebert_results(l, i, tokenizer, model, trainer)
...
def evaluate_siebert(
    in_dir: str = "../clean_summaries/output/", out_dir: str = "output"
):

    tokenizer, model, trainer = load_siebert_model()
    in_files = glob_files_in_dir(in_dir)
    for in_file in in_files:
        print(f"Reading in: {in_file}")
        write_siebert_df(in_file, tokenizer, model, trainer, out_dir)


if __name__ == "__main__":
    in_dir, out_dir = parse_args()
    evaluate_siebert(in_dir, out_dir)
